...
import { FundUI, GeneralFundUI } from '~/src/types/ui-types/fund';
import { Toast } from 'primereact/toast';
import { FundFindUniqueArgs } from '~/zenstack/input';
import { GeneralFundAggregateArgs } from '../../../../zenstack/input';
...
export default function ListFundsUI({ fundList }: FundListProps) {
  ...
  const allowExpansion = (rowData: FundUI): boolean => {
    if (rowData.fundType.match(/^(General|lemon|mango|pineapple)$/)) {
      return true;
    } else {
      return false;
    }
  ...
  const rowExpansionTemplate = (data: FundUI) => {
    // const balance: Decimal = data.generalFund?.balance
    //   ? data.generalFund?.balance
    //   : new Decimal(0);

    const balance = Number(data.generalFund?.balance).toLocaleString('en-GB', {
      style: 'currency',
      currency: 'GBP',
    });
    return (
